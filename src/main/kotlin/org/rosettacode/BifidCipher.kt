...
/**
 * Leave the [square] as a String - we recover it's [dim] X [dim] nature using
 * div and mod.
 */
data class Square(private val square: String) {
    private val dim: Int = sqrt(square.length.toDouble()).toInt()
    fun encode(ch: Char): Pair<Int, Int> = square.indexOf(ch).let { idx -> Pair(idx.div(dim), idx.mod(dim)) }
